...
// Helper code to upload a file (e.g. image, PDF) via multipart/form-data
// Expects `params` to be a FormData object
export function upload(endpoint, formData) {
  return fetch(endpoint, {
    ...
    .then(convertToJSON)
    .catch((error) => {
      throw `UPLOAD request to ${endpoint} failed with error:\n${error}`;
    });
}
